  ...
  return (
    <div className="my-10">
      <div className="flex flex-col items-center gap-5 ">
        ...
        <p className="w-[90%] sm:w-[70%] md:w-[50%] lg:w-[40%] text-center">
        ...
        {pricingTiers.map((val, idx) => {
          return (
            <div
              key={idx}
              className={`border ${
                val.popular ? "bg-gray-800 text-gray-100" : ""
              }  border-black flex flex-col gap-6 rounded-xl sm:w-80 w-full px-7 py-8`}
            >
              <p className="font-semibold flex justify-between">
                <span>{val.title}</span>
                {val.popular && (
                  <span className="text-transparent [backgound-size:200%] bg-clip-text border px-4 py-1 border-gray-300 rounded-md bg-[linear-gradient(to_right,#DD7DDF,#E1CD86,#BBCB92,#71C2EF,#3BFFFF)] ">
                    Popular
                  </span>
                )}{" "}
              </p>
              ...
              <p>
                <span className="text-5xl font-bold ">${val.monthlyPrice}</span>
                /month
              </p>

              <button className="bg-gray-800 mx-auto text-white rounded-lg w-full h-10 flex items-center justify-center">
                {val.buttonText}
              </button>
